...
_COMMON_PORTS = "22,80,443,21,25,53,110,143,993,995,8080,8443,3306,5432,6379,27017,3389,5900,9090,3000,5000,8000,9000"
...
def scan_network(target, all_ports=True):
    ...
    try:
        if not all_ports:
            future = _SCAN_POOL.submit(run_command, [nmap_path, "-T4", "--open", "-p", _COMMON_PORTS, safe_target], 180)
            try:
                out = future.result(timeout=180)
            except TimeoutError:
                return "❌ Fast scan timed out after 180s"
            hosts = parse_nmap_hosts(out)
            summary = _extract_summary(out)
            return format_scan(safe_target, summary, hosts)

        future = _SCAN_POOL.submit(run_command, [nmap_path, "-Pn", "-T4", "--open", "-p-", safe_target], 600)
        try:
            out = future.result(timeout=600)
        except TimeoutError:
            return "❌ Full scan timed out after 600s"
        hosts = parse_nmap_hosts(out)
        summary = _extract_summary(out)
        if not hosts:
